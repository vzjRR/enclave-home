...
class News extends JsonStore {
    ...
    findBySourceMessage(messageId) {
        return this.state.posts.find(post => post.sourceMessageId === messageId) || null;
    }

    /**
     * Posts that came from Discord, optionally for one channel.
     *
     * The sync only ever deletes from this set. A hand-written post has no
     * `source`, so it can never be removed by something happening in
     * Discord -- which is the property that makes an automatic delete safe
     * to run at all.
     */
    fromDiscord(channelId = '') {
        return this.state.posts.filter(post => post.source === 'discord'
            && (channelId ? post.sourceChannelId === channelId : true));
    }
    ...
    /**
     * `origin` carries provenance for a synced post: { source,
     * sourceMessageId, sourceChannelId, createdAt }. Omitted for anything
     * written by hand, which is what keeps the two kinds distinguishable.
     */
    create(input, author, origin = {}) {
        ...
        return this.mutate(state => {
            ...
            const post = {
                ...
                ...(origin.source ? {
                    ...
                    sourceChannelId: origin.sourceChannelId || ''
                } : {}),
                ...
                publishedAt: fields.published ? (origin.createdAt || now) : null
            };
    ...
    /** Remove by Discord message id. Returns true when a post was removed. */
    removeBySourceMessage(messageId) {
        return this.mutate(state => {
            const index = state.posts.findIndex(post => post.source === 'discord'
                && post.sourceMessageId === messageId);
