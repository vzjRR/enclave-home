 ...
 */
const PUBLISH_PLAYERS = process.env.FIVEM_PUBLISH_PLAYER_LIST === 'true';
...
async function fetchDirect(baseUrl) {
    ...
    const [dynamic, players] = await Promise.all([
        ...
        PUBLISH_PLAYERS ? getJson(`${baseUrl}/players.json`) : Promise.resolve(null)
    ]);

    // dynamic.json is the one that decides "is it up". info.json adds nothing
    // the board shows, and players.json is empty on an idle-but-live server.
...
async function fetchListing(joinCode) {
    ...
    return {
        ...
        playerList: PUBLISH_PLAYERS && Array.isArray(data.players)
...
async function refresh(joinCode) {
