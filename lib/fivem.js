...
/**
 * Reduce a raw entry from the enclave-positions resource's positions.json
 * to exactly {x, y}. No id: the map and the player-name list are separate
 * toggles, and a shared id would let anyone turn both on and match a dot to
 * a name by cross-referencing it -- the point of publishing positions
 * without names would be lost. Dropped here even if the resource ever
 * sends more, same defensive posture as publicPlayer() above.
 */
...
// Fixed path, not a setting: it has to match the folder name the
// enclave-positions resource is installed under (see its own README),
// which is also how FXServer names the resource's static file URL.
const POSITIONS_PATH = 'enclave-positions/positions.json';

/** Poll the server's own endpoints. Returns null unless /dynamic.json answers. */
async function fetchDirect(baseUrl, publishPlayers, publishMap) {
    if (!baseUrl) return null;

    const [dynamic, info, players, positions] = await Promise.all([
        ...
        publishMap ? getJson(`${baseUrl}/${POSITIONS_PATH}`) : Promise.resolve(null)
    ]);

    // dynamic.json is the one that decides "is it up". info.json only adds
    // detail rows, players.json is empty on an idle-but-live server, and
    // positions.json is absent unless the enclave-positions resource is
    // installed -- none of the three is allowed to make the server look down.
    ...
    return {
        ...
        playerMap: Array.isArray(positions) ? positions.map(publicPosition) : [],
...
async function fetchListing(joinCode, publishPlayers) {
    ...
    return {
        ...
        playerList: publishPlayers && Array.isArray(data.players)
            ...
            : [],
        // The Cfx.re listing has never carried coordinates and never will --
        // positions only exist on the game server itself. Always empty here,
...
async function refresh(joinCode, publishPlayers, publishMap) {
    ...
    const [direct, listing] = await Promise.all([
        fetchDirect(address, publishPlayers, publishMap),
 ...
 */
async function getStats(joinCode, { publishPlayers = PUBLISH_DEFAULT, publishMap = false } = {}) {
    if (statsCache.payload && Date.now() < statsCache.expiresAt) {
        return statsCache.payload;
    }
    if (inFlight) return inFlight;

    inFlight = refresh(joinCode, publishPlayers, publishMap)
        .then(payload => {
